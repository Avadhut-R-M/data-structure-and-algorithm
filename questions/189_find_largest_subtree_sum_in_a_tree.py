"""
Question - Given a binary tree, task is to find subtree with maximum sum in tree.
...
class Solution:
    def largestSubtreeSumUtil(self,root, total_sum):
        if root == None:
            return 0

        current_sum = (root.data + self.largestSubtreeSumUtil(root.left, total_sum) +
                        self.largestSubtreeSumUtil(root.right, total_sum))

        total_sum[0] = max(current_sum, total_sum[0])
        return current_sum


    def largestSubtreeSum(self,root):
        total_sum = []
        total_sum.append(-9999999999)
        return self.largestSubtreeSumUtil(root,total_sum)
...
if __name__ == "__main__":
    ...
    solution = Solution()
    print(solution.largestSubtreeSum(node))

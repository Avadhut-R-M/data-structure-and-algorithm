"""
Question - Given a Binary Tree, convert it to Binary Search Tree in such a way that keeps the original
...
class Solution:
    def inorderValueFinder(self,root,nodes):
        if root:
            self.inorderValueFinder(root.left, nodes)
            nodes.append(root.data)
            self.inorderValueFinder(root.right, nodes)

    def inorderValueAdder(self,root,nodes):
        if root:
            self.inorderValueAdder(root.left, nodes)
            temp = nodes.pop(0)
            root.data = temp
            self.inorderValueAdder(root.right, nodes)
    # The given root is the root of the Binary Tree
    # Return the root of the generated BST
    def binaryTreeToBST(self, root):
        # code here
        nodes = []
        self.inorderValueFinder(root,nodes)
        nodes.sort()
        self.inorderValueAdder(root,nodes)
